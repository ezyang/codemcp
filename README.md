...
Where `$PROJECT_DIR` is the path to the project you want to work on. Then chat with Claude about what changes you want to make to the project. Every time codemcp makes a change to your code, it will generate a commit.

You can also use codemcp with the Claude web interface at claude.ai by running the SSE server:

```
python -m codemcp.listener
```

This will start a local SSE server on port 8000 that can be connected to from claude.ai.
...
See CONTRIBUTING.md. This project uses pyright for type checking with strict mode enabled. The type checking configuration is in pyproject.toml. We use a few strategies to maintain type safety:

- Typed globals (see codemcp/globals.py for an example). This is necessary to initialize common subsystems like TOML parsing once, but ensure different files can see each other's type signatures. This is the only case of mutable module state in the codebase. Be cautious about adding more.
